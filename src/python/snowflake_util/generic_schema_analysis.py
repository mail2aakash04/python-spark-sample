...
def build_objects_from_df(df: pd.DataFrame) -> Dict[str, Dict[str, Any]]:
    ...
    if name_col is None:
        raise ValueError("Could not detect object/table name column in dataframe. Columns found: " + ", ".join(df_norm.columns))
...
def generate_html_report(results: Dict[str, Any], product_name: str, output_dir: str) -> str:
    ...
    if readiness_score >= 80:
        readiness_level = "🟢 EXCELLENT"; readiness_color = "#28a745"
    elif readiness_score >= 70:
        readiness_level = "🟡 GOOD"; readiness_color = "#ffc107"
    elif readiness_score >= 60:
        readiness_level = "🟠 MODERATE"; readiness_color = "#fd7e14"
    else:
        readiness_level = "🔴 NEEDS WORK"; readiness_color = "#dc3545"
...
def main():
    ...
    for sf_name, sf_info, sole_name, sole_info, score, overlap_data in matches:
        ...
        if overlap_data['overlap_ratio'] >= 0.9:
            migration_complexity = 'direct_copy'; confidence = 'high'
        elif overlap_data['overlap_ratio'] >= 0.7:
            migration_complexity = 'column_mapping'; confidence = 'high' if overlap_data['overlap_ratio'] >= 0.8 else 'medium'
        else:
            migration_complexity = 'complex_restructure'; confidence = 'low'
    ...
    results = {
        'summary': {
            ...
            'complex_restructure_objects': len([r for r in object_results if r['migration_complexity'] == 'complex_restructure'])
        },
    ...
    json_file = os.path.join(args.output_dir, f"{args.product_name}_Schema_Analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
